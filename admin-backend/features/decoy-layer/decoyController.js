import DecoyLayer from '../models/decoyLayer.js';

// CREATE
export const createObscuraLogic = async (req, res) => {
  try {
    const newLogic = new DecoyLayer(req.body);
    await newLogic.save();
...
export const updateObscuraLogic = async (req, res) => {
  try {
    const { id } = req.params;
    const updated = await DecoyLayer.findByIdAndUpdate(id, req.body, { new: true });
...
export const deleteLogic = async (req, res) => {
  try {
    const { id } = req.params;
    await DecoyLayer.findByIdAndDelete(id);
...
export const getObscuraLogics = async (req, res) => {
  try {
    const logics = await DecoyLayer.find().sort({ createdAt: -1 });
...
export const toggleLogic = async (req, res) => {
 try {
    const logic = await DecoyLayer.findById(req.params.id);
...
export const editLogic = async (req, res) => {
  try {
    ...
    const updated = await DecoyLayer.findByIdAndUpdate(id, updates, {
      new: true,
    });
