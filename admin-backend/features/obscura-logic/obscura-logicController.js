...
export const createObscuraLogic = async (req, res) => {
  try {
    const newLogic = new ObscuraLogic(req.body);
    await newLogic.save();
...
export const updateObscuraLogic = async (req, res) => {
  try {
    const { id } = req.params;
    const updated = await ObscuraLogic.findByIdAndUpdate(id, req.body, { new: true });
...
export const deleteObscuraLogic = async (req, res) => {
  try {
    const { id } = req.params;
    await ObscuraLogic.findByIdAndDelete(id);
...
export const getObscuraLogics = async (req, res) => {
  try {
    const logics = await ObscuraLogic.find().sort({ createdAt: -1 });
...
export const toggleLogic = async (req, res) => {
 try {
    const logic = await ObscuraLogic.findById(req.params.id);
...
export const editLogic = async (req, res) => {
  try {
    ...
    const updated = await ObscuraLogic.findByIdAndUpdate(id, updates, {
      new: true,
    });
...
export const deleteLogic = async (req, res) => {
  try {
    const logic = await ObscuraLogic.findByIdAndDelete(req.params.id);
