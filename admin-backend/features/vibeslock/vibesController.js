import VibeLock from '../models/VibesLock.js';

// CREATE
export const createLogic = async (req, res) => {
  try {
    const newLogic = new VibeLock(req.body);
    await newLogic.save();
...
export const updateLogic = async (req, res) => {
  try {
    const { id } = req.params;
    const updated = await VibeLock.findByIdAndUpdate(id, req.body, { new: true });
...
export const deleteLogic = async (req, res) => {
  try {
    const { id } = req.params;
    await VibeLock.findByIdAndDelete(id);
...
export const getLogics = async (req, res) => {
  try {
    const logics = await VibeLock.find().sort({ createdAt: -1 });
...
export const toggleLogic = async (req, res) => {
 try {
    const logic = await VibeLock.findById(req.params.id);
...
export const editLogic = async (req, res) => {
  try {
    ...
    const updated = await VibeLock.findByIdAndUpdate(id, updates, {
      new: true,
    });
